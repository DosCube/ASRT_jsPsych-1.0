function expStart(subject, session) {
    ...
    const secondTrialProperties = {... trialProperties, pre_target_duration: rsi, target_color: secondTrialImage}; //set this to have rsi, it effectively replaces random trial properties I think, badly named
    const secondTrialWithDelayProperties = {... trialProperties, pre_target_duration: initialDelay, target_color: secondTrialImage}; //needed to replace first trial props which is strangely named
    ...
    function randomRepeat(actualRandom, blockNumber) {

        if(blockNumber >= blockToBeginUsingSecondImage){
            return {
                timeline: [secondRandomIncorrectTrialProperties],
    ...
    if (numberOfPracticeBlocks > 0) {
        ...
        for (let blockNumber = 1; blockNumber < numberOfPracticeBlocks+1; blockNumber++) {
            actualRandom = randomStimulusProc(blockNumber,1,1,1) //longer delay before first element
            timeline.push(actualRandom);
            insertRepetition(randomRepeat(actualRandom, 1));
            for (let l = 2; l < (numberOfBlockElements+1); l++) { //now 85 practice element in one block
                actualRandom = randomStimulusProc(blockNumber,l,0,1);
                timeline.push(actualRandom);
                insertRepetition(randomRepeat(actualRandom, 1));
            }
            timeline.push(feedback);
        
            if (blockNumber!==numberOfPracticeBlocks){
    ...
    for (let blockNumber = 1; blockNumber < numberOfBlocks+1; blockNumber++) {

        /* first five random stimuli at the beginning of the block*/

        actualRandom = randomStimulusProc(blockNumber,1,1,0) //before first element, longer delay
        timeline.push(actualRandom);
        insertRepetition(randomRepeat(actualRandom, blockNumber));
        for (let l = 2; l < 6; l++) {
            actualRandom = randomStimulusProc(blockNumber,l,0,0)
            timeline.push(actualRandom);
            insertRepetition(randomRepeat(actualRandom, blockNumber));
        }
        ...
        for (let k = 0; k < numberOfSequenceRepetitions; k++) { //repeat 8-elements sequence 10 times
            for (let n = 0; n < 4; n++) { //repeat pattern + repeat random
                ...
                actualRandom = randomStimulusProc(blockNumber,n+n+6+(k*8), 0, 0)
                timeline.push(actualRandom);
                insertRepetition(randomRepeat(actualRandom, blockNumber));

                let dataForPattern = {p_or_r: "P", block: blockNumber, first_response: 1, trial_number: n+n+7+(k*8), sequence: usedSequenceString, is_practice: 0} //output parameters for pattern stimuli
                ...
                if(blockNumber < blockToBeginUsingSecondImage){
                    ...
                    let patternTrialProcTwo = {
                        timeline: [patternTrialPropertiesTwo],
                        timeline_variables: [{stimulus: [0, usedSequence[n]], data: dataForPattern}]
                    }
                    timeline.push(patternTrialProcTwo)
                    ;
                    let patternIncorrectTrialProcTwo = new IncorrectTrialProcs([patternIncorrectTrialPropertiesTwo], [{
                        stimulus: [0, usedSequence[n]],
                        data: dataForPattern
                    }]);
                    insertRepetition(patternIncorrectTrialProcTwo)
                }
            }
        }
